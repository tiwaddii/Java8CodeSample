...
public class DeptMaxEmpCount {
	...
	public static void main(String[] args) {
		...
		List <Employee> empList = Arrays.asList(new Employee(1, "Alice", "HR", 30, "F", 30000),
		  ...
		  Map.Entry<String, Long> deptMaxCount  = empList.stream()
				                                  .collect(Collectors.groupingBy(Employee::getDepName, Collectors.counting()))
		 			                              .entrySet().stream().max(Map.Entry.comparingByValue()).get();
		  
		  System.out.println("The department who is having maximum number of employee: "+ deptMaxCount);
	}
